...
const handleSaleEvent = async (item) => {
  const {tokenId, marketplace, price, fromAddress, toAddress, claimed, dna} = item;
...
const handleOrderCreatedEvent = async (item) => {
  const {tokenId, marketplace, price, fromAddress, claimed, dna} = item;
  const embedDetails = {
    tokenId,
    marketplace,
    price,
  ...
  if (!claimed) {
    await Promise.all([
      ...
      DiscordClient.dnaChannels[dna].send({embeds: [DiscordClient.createEggListingEmbedMsg(embedDetails)]})
    ]);
  } else {
    await Promise.all([
      DiscordClient.channels.listings.send({embeds: [DiscordClient.createListingEmbedMsg(embedDetails)]}),
      DiscordClient.dnaChannels[dna].send({embeds: [DiscordClient.createListingEmbedMsg(embedDetails)]})
    ]);
  }
};
...
const handleEventCreated = async (payload) => {
  payload.items.map(async (item) => {
    ...
    item.dna = (await getCloneAttributes(item.tokenId)).find(attribute => attribute.trait_type === 'DNA').value.toLowerCase();
