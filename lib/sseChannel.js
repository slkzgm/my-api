...
const { web3, contract } = require('./contracts/eggs');
...
const run = async () => {
  try {
    ...
    const testChannel = await DiscordClient.getChannelById('1076248623731462214');
    ...
    client.onItemListed('clonex', async (event) => {
      ...
      if (!claimed) {
        const embed = DiscordClient.createEmbedMsg({
          id,
