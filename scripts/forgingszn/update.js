...
const ForgingSznCollection = require('../../lib/forgingsznDatabase');
...
const retrieveSupply = async (contract, id) => {
  try {
    const [minted, uri] = await Promise.all([
      contract.methods.generalSupply(id).call(),
      contract.methods.tokenURIs(id).call()
    ]);
    const data = (await axios.get(`https://ipfs.io/ipfs/${uri.slice(7)}`)).data;
    const name = data.name;
    const dna = data.attributes.filter(attr => attr.trait_type === 'Collection')[0].value;
...
const updateMintedSupply = async () => {
  ...
  await Promise.all(res.map(e => ForgingSznCollection.updateSupply(e._id, e.minted)));
